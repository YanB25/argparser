...
TEST(ArgparserCommand, ShouldParseRootAllocatedFlag)
{
    auto parser = argparser::new_parser();
    EXPECT_TRUE(parser->flag("--i", "", "", "0"));
    EXPECT_TRUE(parser->flag("--u", "", "", "0"));
...
TEST(ArgparserCommand, ShouldFollowCommand)
{
    int64_t i;
    auto parser = argparser::new_parser();
...
TEST(ArgparserCommand, ShouldFollowLongCommandForAllocatedFlag)
{
    auto parser = argparser::new_parser();
    auto &sub_parser_1 = parser->command("one");
    auto &sub_parser_2 = sub_parser_1.command("two");
    ...
    auto &sub_parser_5 = sub_parser_4.command("five");
    auto &sub_parser_6 = sub_parser_5.command("six");
    EXPECT_TRUE(sub_parser_6.flag("--i", "", "", "0"));
    ...
    auto &store = parser->store();
    ASSERT_TRUE(store.has("--i"));
    EXPECT_EQ(store.get("--i").to<int64_t>(), 12345);
}
...
TEST(ArgparserCommand, SameCommandSameParser2)
{
    auto parser = argparser::new_parser();
    auto &sub_parser_1 = parser->command("same");
    auto &sub_parser_2 = parser->command("same");
    EXPECT_TRUE(sub_parser_1.flag("--i", "", "", "0"));
    EXPECT_TRUE(sub_parser_2.flag("--j", "", "", "0"));
    ...
    auto &store = parser->store();
    ASSERT_TRUE(store.has("--i"));
    ASSERT_TRUE(store.has("--j"));
    EXPECT_EQ(store.get("--i").to<int64_t>(), 12345);
    EXPECT_EQ(store.get("--j").to<uint64_t>(), 23456);
}
...
TEST(ArgparserCommand, RootCommandNotAcceptSubFlag2)
{
    uint64_t j;
    auto parser = argparser::new_parser();
    auto &sub_parser = parser->command("same");
...
TEST(ArgparserCommand, RootCommandNotAcceptSubFlag3)
{
    int64_t i;
    auto parser = argparser::new_parser();
    auto &sub_parser = parser->command("same");
    EXPECT_TRUE(parser->flag(&i, "--i", "", "", "0"));
    EXPECT_TRUE(sub_parser.flag("--j", "", "", "0"));
...
TEST(ArgparserCommand, RootCommandOnlyRequireRootFlag)
{
    ...
    auto &store = parser->store();
    ASSERT_TRUE(store.has("--i"));
    EXPECT_FALSE(store.has("--j"));
    EXPECT_EQ(store.get("--i").to<int64_t>(), 12345);
}
TEST(ArgparserCommand, RootCommandOnlyRequireRootAllocatedFlag)
{
    auto parser = argparser::new_parser();
    auto &sub_parser = parser->command("same");
    EXPECT_TRUE(parser->flag("--i", "", ""));
    EXPECT_TRUE(sub_parser.flag("--j", "", ""));
...
TEST(ArgparserCommand, FailedIfRootCommandWithSubFlagForAllocatedFlag)
{
    uint64_t j;
    auto parser = argparser::new_parser();
    auto &sub_parser = parser->command("same");
...
TEST(ArgparserCommand, FailedIfRootCommandWithSubFlagForAllocatedFlag2)
{
    int64_t i;
    auto parser = argparser::new_parser();
    auto &sub_parser = parser->command("same");
...
TEST(ArgparserCommand, SubCommandOnlyRequireSubAllocatedFlag)
{
    int64_t i;
    auto parser = argparser::new_parser();
    auto &sub_parser = parser->command("same");
    ...
    auto& store = parser->store();
    ASSERT_TRUE(store.has("--j"));
    EXPECT_FALSE(store.has("--i"));
    EXPECT_EQ(store.get("--j").to<int64_t>(), 23456);
}
...
TEST(ArgparserCommand, FailedIfSubCommandWithRootAllocatedFlag)
{
    uint64_t j;
    auto parser = argparser::new_parser();
    auto &sub_parser = parser->command("same");
...
TEST(ArgparserCommand, StoreOnlySeeCurrentCommandFlags)
{
    auto parser = argparser::new_parser();
    auto &sub_parser = parser->command("same");
    EXPECT_TRUE(parser->flag("--i", "", ""));
    ...
    const char *arg[] = {"./argtest", "same", "--j=12345"};
    EXPECT_TRUE(parser->parse(sizeof(arg) / sizeof(arg[0]), arg));
    ...
    EXPECT_EQ(store.get("--j").to<uint64_t>(), 12345);
}
TEST(ArgparserCommand, ExtraFlagsIsAnError)
{
    auto parser = argparser::new_parser();
    auto &sub_parser = parser->command("same");
    EXPECT_TRUE(parser->flag("--i", "", ""));
    ...
    const char *arg[] = {"./argtest", "--i=246810", "--j=12345"};
    EXPECT_FALSE(parser->parse(sizeof(arg) / sizeof(arg[0]), arg));
}
TEST(ArgparserCommand, ExtraFlagsIsAnError2)
{
    auto parser = argparser::new_parser();
    auto &sub_parser = parser->command("same");
    EXPECT_TRUE(parser->flag("--i", "", ""));
    ...
    const char *arg[] = {"./argtest", "same", "--i=246810", "--j=12345"};
    EXPECT_FALSE(parser->parse(sizeof(arg) / sizeof(arg[0]), arg));
}
